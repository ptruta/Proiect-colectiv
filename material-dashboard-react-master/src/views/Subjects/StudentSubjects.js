...
class SubjectForm extends React.Component {
  ...
  createOptionalMenuItems() {
    let items = [];
    for (let i = 0; i < this.state.optionals.length; i++) {
      items.push(
        <MenuItem value={this.state.optionals[i]}>
          {this.state.optionals[i]}
        </MenuItem>
      )
    }
      return items;
  ...
  selectionOptional(event) {
    this.setState({ selectedOptional: event.target.value });
  }
