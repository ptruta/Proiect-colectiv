...
import classbookService from 'services/classbook-service.js';
...
export default class StudentSubjectScreen extends React.Component{
    ...
    constructor(props){
        ...
        this.state = {
            ...
            news: [],
            __rawData : {},
            laboratoryEntry: undefined,
            seminaryEntry: undefined
        }
    }

    async fetchAttendances() {
        const student = await classbookService.getStudentDetails();
        const classbook = await classbookService.getStudentClassBook(this.state.subjectId);
        let laboratoryEntry = undefined;
        let seminaryEntry = undefined;
        if (classbook.laboratoryFrequency > -1) {
            laboratoryEntry = this.mapLabEntries(student, classbook);
        }
        if (classbook.seminaryFrequency > -1) {
            seminaryEntry = this.mapSemEntries(student, classbook);
        }
        this.setState({
            laboratoryEntry: laboratoryEntry,
            seminaryEntry: seminaryEntry
        });
      }
  
      mapLabEntries(student, classbook) {
        let laboratory = {
            students: [{
                entries: classbook.entries.filter(entry => entry.classType == "LABORATORY"),
                student: student
            }],
            classFrequency: classbook.laboratoryFrequency,
        };
        return laboratory;
      } 
  
    mapSemEntries(student, classbook) {
        console.log("msg (dar nu lucru) 2", classbook);
        let seminar = {
            students: [{
                entries: classbook.entries.filter(entry => entry.classType == "SEMINARY"),
                student: student
            }],
            classFrequency: classbook.seminaryFrequency,
        };
        return seminar;
    } 
    ...
    render(){
        ...
        return (
            ...
            {this.state.laboratoryEntry ? <div> 
                <ClassbookTable editable={false} forAttendance={true} classbook={this.state.laboratoryEntry} classType="LABORATORY" />
                <ClassbookTable editable={false} forAttendance={false} classbook={this.state.laboratoryEntry} classType="LABORATORY" />
             </div> : null}
             {this.state.seminaryEntry ? <div> 
                <ClassbookTable editable={false} forAttendance={true} classbook={this.state.seminaryEntry} classType="SEMINARY" />
                <ClassbookTable editable={false} forAttendance={false} classbook={this.state.seminaryEntry} classType="SEMINARY" />
             </div> : null}
